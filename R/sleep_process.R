...
#'
#' @param x Dataframe with a vector of times (24-hour decimal format) and sleep/wake states (0 = wake, 1 = sleep).
#' @param sleep_var String - name of sleep variable in x dataframe
#' @param time_var String - name of time variable in x dataframe
...
#' @noRd
#'
sleep24Summary <- function(x, sleep_var, time_var, epoch_length, noon_to_noon){

  # construct days of observation
  x$day <- epochDays(x[[time_var]], noon_to_noon = noon_to_noon)

  ## extract day runs ##
  day_rle <- rlFunc(x$day)

  ## calculate duration of each day ##
  # subtract start times from end times
  day_rle$duration <- x[[time_var]][day_rle$end] -
    x[[time_var]][day_rle$start] # subtract all start times
  ...
  day_stats <- do.call("rbind", lapply(day_rle$value, function(i){
    ...
    sleep_rle <- rlFunc(x[[sleep_var]][x$day==i]) # rle for sleep within given observation day
    ...
    sleep_ons <- x[x$day==i,][start_inds, time_var] # sleep onsets in original time scale
    sleep_offs <- x[x$day==i,][end_inds, time_var] + epoch_length # sleep offsets in original scale; note that sleep offset occurs after the index of the final sleep epoch
  ...
  ## TODO consider check global sleep runs against day boundaries, to identify sleep
...
sleepSummary <- function(x, sleep_var, time_var){
  ...
  # check that sleep_var and time_var are in x #
  if(!sleep_var %in% names(x)){
    stop("Value for sleep_var is not in data.frame x")
  }
  if(!time_var %in% names(x)){
    stop("Value for time_var is not in data.frame x")
  }

  # check for NAs in sleep or time vectors
  if(sum(is.na(x[[sleep_var]])) > 0){
    stop("sleep_var in data.frame x cannot have NAs")
  }

  if(sum(is.na(x[[time_var]])) > 0){
    stop("time_var in x cannot have NAs")
  }

  # check that sleep_var is in correct format (either 0 or 1)
  if(sum(!unique(x[[sleep_var]] %in% c(0,1))) > 0){
    stop("sleep_var in data.frame x must be in binary format (0 = wake, 1 = sleep)")
  }
  ...
  if(!is(x[[time_var]], "numeric") || sum(x[[time_var]] < 0) > 0){
    stop("time_var in data.frame x must be a numeric vector representing cumulative time in 24-hour decimal format.")
  }

  # extract epoch length - return error if epochs are not evenly spaced #
  epoch_lengths <- unique(round(diff(x[[time_var]]), 10)) # round to avoid floating point error
  ...
  sleep_offs <- x[end_inds, time_var] + epoch_lengths # sleep offsets in original scale; note that sleep offset occurs after the index of the final sleep epoch
  sleep_durs <- sleep_offs - sleep_ons # sleep durations in original time units
  sleep_mids <- sleep_ons + sleep_durs/2 # sleep midpoints in original time scale
  ...
  n2n_df <- sleep24Summary(x=x, sleep_var=sleep_var, time_var=time_var,
                           epoch_length=epoch_lengths, noon_to_noon = TRUE) # noon-to-noon
  m2m_df <- sleep24Summary(x=x, sleep_var=sleep_var, time_var=time_var,
